 ...
 */
class WC_REST_Emerch_Products_Controller extends WC_REST_Legacy_Products_Controller {
	...
	protected function prepare_objects_query( $request ) {
	    ...
	    //THIS METHOD CREATES WP_QUERY PARAMETERS
        //print_r($request);die;
        ...
        // Filter product type by attributes
        if ( ! empty( $request['attribute'] ) && ! empty( $request['attribute_term'] ) ) {
            //$tax_query = [];
            $attribute_names_exploded = explode(',', $request['attribute']);

            $attribute_term_from_request = $request['attribute_term'];

            foreach ($attribute_names_exploded as $attribute_name) {

                $terms_ids = [];

                $terms_temp = $request['attribute_term'];
                if (!empty($terms_temp)) {
                    foreach ($terms_temp as $term_id) {
                        $term_instance = get_term($term_id);

                        if ($attribute_name === $term_instance->taxonomy) {
                            $terms_ids[] = $term_id;
                            unset($attribute_term_from_request[$term_id]);
                        }
                    }
